...
public static class SpriteSheetProcessorExtended
{
    ...
    {
        ExtendedTextureAtlas extendedAtlas = TextureAtlasProcessorExtended.Process(graphicsDevice, aseFile, onlyVisibleLayers, includeBackgroundLayer, includeTilemapLayers, mergeDuplicates, borderPadding, spacing, innerPadding);
        ExtendedSpriteSheet extendedSpriteSheet = new();

        //  Need to change texture atlas processor to create from raw so we don't have to
        //  generate the raw spritesheet stuff here twice

        extendedSpriteSheet.TextureAtlas = extendedAtlas;
